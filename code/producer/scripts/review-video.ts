...
class AgyReviewProvider implements ReviewProvider {
  ...
  private async prompt(req: ReviewRequest): Promise<string> {
    ...
    return [
      ...
      "6. 方向 Tab 高亮：顶部方向 tab 高亮是否与当前条目所属方向一致（错位标 high）",
      "7. 字幕条遮挡：内嵌字幕条是否遮挡标题/数据卡/关键数字（遮挡标 high）",
      "8. 素材图质量：素材图是否等比无拉伸、无模糊、无变形（异常标 medium）",
      "9. 四宫格排版：四宫格条目是否断行/错位/溢出边界（异常标 medium）",
      "",
      "【输出契约】严格只输出一个 JSON 对象：",
