...
export type VideoStyle =
  ...
  | "newspaper" // #6 报纸头条
  | "journal" // #9 手账日记
  | "news-paper" // #10 聚合页纸媒（对标《AI早报》；morning=浅皮，evening 自动切深皮）
  | "news-paper-dark"; // #11 聚合页纸媒·强制深皮（对标《AI晚报》）
...
export type VideoTemplate =
  ...
  | "news-paper" // 聚合页纸媒（对标《AI早报/晚报》：T1-T6 模板 + 插入素材三档，见 compositions/news-paper）
...
export interface VideoBlock {
  ...
  subtitle?: string;
  /** ===== news-paper 模板扩展（全部可选；缺省走自动路由，见 compositions/news-paper）===== */
  /** 模板覆盖（T1-T6）：缺省时按块类型 + 要点数自动选择（3→T6 1×3 / 4→T3 2×2 / 5→T4 3+2 / ≥6→2×3） */
  template?: "T1" | "T2" | "T3" | "T4" | "T5" | "T6";
  /** 卡片 icon 语义（缺省按卡内序号轮换四色 + 图标名） */
  icon?: { color?: "red" | "orange" | "green" | "cyan"; glyph?: string };
  /** 证据插入屏规格（对齐 doc/09 S1 全屏 / S2 大幅 / S3 居中卡）；image/video 块缺省 card/full */
  evidence?: {
    scale?: "full" | "wide" | "card";
    /** 插入屏说明行（缺失回退 media.caption / content） */
    caption?: string;
  };
}
