...
/** run 目录固定结构（计划 §2.3/§2.5）：runs/YYYY-MM-DD/<stream>-<edition>-<slug>/ */
export interface RunDirPaths {
  root: string;
  configDir: string;
  configPath: string; // config/content.json
  audioDir: string; // audio/<i>.wav
  timelineDir: string;
  timelinePath: string; // timeline/timeline.json
  subtitlePath: string; // timeline/subtitle.srt
  rendersDir: string; // renders/short.mp4 | long.mp4
  reviewDir: string;
  packageDir: string;
}

/** 由 run-dir 展开固定子路径（仅拼路径，不校验存在性） */
export const runDirPaths = (runDir: string): RunDirPaths => ({
  root: runDir,
  configDir: `${runDir}/config`,
  configPath: `${runDir}/config/content.json`,
  audioDir: `${runDir}/audio`,
  timelineDir: `${runDir}/timeline`,
  timelinePath: `${runDir}/timeline/timeline.json`,
  subtitlePath: `${runDir}/timeline/subtitle.srt`,
  rendersDir: `${runDir}/renders`,
  reviewDir: `${runDir}/review`,
  packageDir: `${runDir}/package`,
});
