---
name: motion-media-handoff
description: 项目内编排技能（Motion 项目专属）：负责素材、音频、字幕、时间轴和渲染交接。编排已锁定的原子技能（media-use、hyperframes-audio、hyperframes-cli、hyperframes-core），管理 runs/<date>/<run>/ 内的资产清单、音轨一致性、渲染门与交接包。不复制其正典表格。注意：与 HeyGen 官方原子技能 hyperframes-media（TTS/BGM/SFX 音频引擎）区分，本技能只做项目级编排与交接门，不产出音频。
---

# Motion Media Handoff — 素材/音频/字幕/渲染交接

> 原名 `hyperframes-media`，为避免与 HeyGen 官方同名原子技能冲突而改名。
> 需要真正生成 TTS/BGM/SFX 时，请用官方 `hyperframes-media`；需要校验与交接时用本技能。
