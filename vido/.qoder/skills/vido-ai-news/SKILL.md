...
```bash
npm run research   # 15 信源（AI 10 源 + 其他新闻 5 源）→ research/today/raw.json
npm run score      # 打分+去重 → research/today/top.md（AI Top3 + 其他 Top3 推荐卡）
```
...
```bash
npm run tts        # → out/voiceover/{i}.wav（CosyVoice2 优先，edge-tts 暂代）
npm run timeline   # → out/timeline.json（唯一时间事实源）
npm run srt        # → out/subtitle.srt
```

edge-tts 偶发失败用补跑模式：单段重试 8 次×3s（详见 docs/workflow.md 第三节）。

### 6. 生成 HyperFrames 合成并渲染（双格式）

```bash
node scripts/gen-hyperframes.ts                            # 竖屏 1080×1920 → hyperframes/ai-news/
node scripts/gen-hyperframes.ts --orientation long         # 横屏 1920×1080 → hyperframes/ai-news-long/
...
cd ..\ai-news-long
npx hyperframes render --output ../../out/ai_news_long.mp4   # B站横屏版
```
...
- 交付 out/ai_news_short.mp4；横屏版可改 data-width/height 为 1920×1080 重渲
